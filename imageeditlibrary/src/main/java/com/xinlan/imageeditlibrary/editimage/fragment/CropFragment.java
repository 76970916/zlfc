...
public class CropFragment extends BaseEditFragment {
    ...
    @BindView(R2.id.ratio_list_group)
    LinearLayout ratioList;
...
//        activity.mainImage.setScaleEnabled(false);// 禁用缩放
        ...
        activity.mainImage.post(new Runnable() {
            @Override
            public void run() {
                StickerItem stickerItem = activity.mainImage.getItem();
                Drawable drawable = new BitmapDrawable(stickerItem.bitmap);
                activity.mainImage.setDrawable(drawable);
